...
class labelgen_api extends restful_api {
	...
	private function get_user_id_from_password($pw) {
		...
		if ($result && is_array($result)) {
			if ($this->decrypt($pw, $result[0]->password)) {
				...
				//*** User login session gsk ***
				
				$_SESSION['wp_user_id'] = $this->user_id;
				$_SESSION['wp_user_name'] = $result[0]->name;
				$user_id = $_SESSION['wp_user_id']; //this has to be set when the user logs on your website 
				wp_set_auth_cookie( $user_id);
				// \*** End user login session gsk ***
			} else {
	...
	private function get_user_id_from_secret() {
		...
		if ($results) {
			...
			if ($saved_digest == $input_digest) {
				...
				throw new Exception("Are you sure you want to do that?");  //save form confirm message gsk...
			} 										
		} else {
	...
	protected function check_credentials() {
		if( isset( $_SESSION['wp_user_id'] ) ){
			return json_encode(array('success'=>true, 'message'=>true));
		}else{return json_encode(array('success'=>true, 'message'=>false));}
		...
		switch ($this->method) {
			case ('GET'):
				...
				$fields = array('id', 'option_name', 'owner', 'price', 'location');
                                if( isset( $_SESSION['wp_user_id'] ) ){
					$this->user_id = $_SESSION['wp_user_id'];
				}
				if ($this->user_id) {
					global $wpdb;
					$wpdb->query(
